...
function createSubtitleCard(subtitle) {
    ...
    // Priority: imageData (direct) > imageUrl (external) > placeholder
    if (subtitle.imageData) {
        imageUrl = subtitle.imageData;
        console.log('Using direct image for:', subtitle.title);
    } else if (subtitle.imageUrl) {
        imageUrl = subtitle.imageUrl;
        console.log('Using image URL for:', subtitle.title);
    } else if (subtitle.imageChunkIds && subtitle.imageChunkIds.length > 0) {
        // Chunked image - load asynchronously
        ...
        // Load chunked image asynchronously
        setTimeout(async () => {
            try {
                const loadedImage = await downloadFileFromChunks(
                    subtitle.imageChunkIds,
                    'image',
                    subtitle.imageMimeType || 'image/jpeg'
                );
                const img = document.querySelector(`[data-id="${subtitle.id}"] .subtitle-thumbnail img`);
                if (img) {
                    img.src = loadedImage;
                    console.log('Loaded chunked image for:', subtitle.title);
                }
            } catch (error) {
                console.error('Failed to load chunked image:', error);
            }
        }, 100);
    }
        ...
        <div class="subtitle-card" data-id="${subtitle.id}">
            <div class="subtitle-thumbnail">
                <img src="${imageUrl}" 
                     alt="${subtitle.title}" 
                     loading="lazy" 
                     onerror="this.src='data:image/svg+xml,%3Csvg width=\\'300\\' height=\\'200\\' xmlns=\\'http://www.w3.org/2000/svg\\'%3E%3Crect width=\\'300\\' height=\\'200\\' fill=\\'%23222\\'/%3E%3Ctext x=\\'50%25\\' y=\\'50%25\\' fill=\\'%23666\\' font-size=\\'14\\' text-anchor=\\'middle\\' dy=\\'.3em\\'%3ENo Image%3C/text%3E%3C/svg%3E'">
...
async function downloadSubtitle(id) {
    ...
    try {
        ...
        // Increment download count (don't block if not logged in)
        try {
            await db.collection('subtitles').doc(id).update({
                downloads: firebase.firestore.FieldValue.increment(1)
            });
        } catch (err) {
            console.log('Could not update download count (not logged in)');
        }
